...
class SMOTE(nn.Module):
    def __init__(self, distance="euclidian", hidden_dim=512, k=5):
        super().__init__()
        self.newindex = 0
        self.k = k
    ...
    def k_neighbors(self, euclid_distance, k):
        nearest_idx = torch.zeros(
            (euclid_distance.shape[0], euclid_distance.shape[0]), dtype=torch.int64
        )
    ...
    def forward(self, X, y):
        ...
        for i in range(len(occ)):
            ...
            if i != dominant_class:
                ...
                candidates = X[y == i]
                xs = self.generate(candidates, N, self.k).to(X.device)
                X = torch.cat((X, xs))
                ys = torch.ones(xs.shape[0]) * i
                y = torch.cat((y, ys.to(y.device)))
        return X, y
