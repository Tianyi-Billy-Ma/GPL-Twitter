...
class HeCo(nn.Module):
    def __init__(self, model_config, num_node_types=3):
        ...
        self.dropout = nn.Dropout(p=model_config.additional.dropout)

        MappingModelClass = model_config.MappingModelClass
        MappingModelConfig = model_config.MappingModelConfig

        MPModelClass = model_config.MPModelClass
        MPModelConfig = model_config.MPModelConfig

        SCModelClass = model_config.SCModelClass
        SCModelConfig = model_config.SCModelConfig

        ContrastModelClass = model_config.ContrastModelClass
        ContrastModelConfig = model_config.ContrastModelConfig

        self.mappings = nn.ModuleList()
        for _ in range(num_node_types):
            self.mappings.append(globals()[MappingModelClass](**MappingModelConfig))
        self.mp = globals()[MPModelClass](**MPModelConfig)
...
class Contrast(nn.Module):
    ...
    def forward(self, z_mp, z_sc, pos):
        ...
        l1 = self.sim_loss(z_proj_mp, z_proj_sc)
        l2 = self.sim_loss(z_proj_sc, z_proj_mp)
